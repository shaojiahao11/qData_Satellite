...
# Pure ASCII, no Unicode, compatible with Windows PowerShell and PowerShell Core.
...
$testResult = ssh -o ConnectTimeout=5 -o BatchMode=yes ${ServerUser}@${ServerHost} "echo 'AUTH_OK'" 2>$null
...
scp -o ConnectTimeout=10 -o StrictHostKeyChecking=no $PublicKeyPath ${ServerUser}@${ServerHost}:${TempPubKeyRemote}
if ($LASTEXITCODE -ne 0) {
Write-Host "ERROR: Failed to upload public key!" -ForegroundColor Red
exit 1
}

# Step 5: Configure authorized_keys on remote server (SAFE SINGLE-LINE VERSION)
Write-Host "Configuring SSH on remote server..." -ForegroundColor Magenta

# Use single-line command with && to avoid set -e and quoting issues
$SetupScript = "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat '$TempPubKeyRemote' >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys && rm -f '$TempPubKeyRemote' && echo 'SSH key installed.'"

# Wrap entire command in single quotes to prevent shell parsing issues
$sshCmd = "ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no ${ServerUser}@${ServerHost} '$SetupScript'"
...
$finalTest = ssh -o ConnectTimeout=5 -o BatchMode=yes ${ServerUser}@${ServerHost} "echo 'FINAL_OK'" 2>$null
...
Write-Host "  ssh ${ServerUser}@${ServerHost}" -ForegroundColor Yellow
}